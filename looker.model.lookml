...
- explore: scheduled_plan
  fields: [ALL_FIELDS*, -user.roles, -spd_user.roles]
  ...
  joins:
    ...
    - join: scheduled_plan_destination
      sql_on: |
        scheduled_plan_destination.scheduled_plan_id = scheduled_plan.id
      relationship: one_to_many
    ...
    - join: scheduled_job
      sql_on: |
        scheduled_job.scheduled_plan_id = scheduled_plan.id
      relationship: one_to_many

    - join: scheduled_job_stage
      sql_on: |
        ${scheduled_job.id} = scheduled_job_stage.scheduled_job_id
      relationship: one_to_many
